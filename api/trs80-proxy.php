 ...
 * api/trs80-proxy.php
 *
 * CORS proxy for the IlluminatOS TRS-80 app (apps/TRS80.js).
 *
 * Mirrors the shape of api/c64-proxy.php and api/dosbox-proxy.php. The
 * TRS-80 app's curated library references Internet Archive items; the app
 * uses IA's advancedsearch + metadata APIs to discover the actual
 * .dsk/.cmd/.cas filename and then downloads it. IA's CORS posture has
 * been historically inconsistent for third-party browser embeds, so the
 * request is routed through this proxy to make the integration
 * deterministic regardless of upstream header changes.
 *
 * Security hardening (same posture as c64-proxy.php / dosbox-proxy.php):
 ...
 *
 * URL format: GET /api/trs80-proxy.php?url=<urlencoded-upstream-url>
 */
...
// SSRF allowlist — hosts that legitimately serve TRS-80 software / metadata.
...
// vary per item. Accept any subdomain following the ia<digits>.us.archive.org
// pattern, plus any other *.archive.org host.
if (!$hostAllowed && (
    preg_match('/^ia\d+\.us\.archive\.org$/', $host) ||
    preg_match('/\.archive\.org$/', $host)
)) {
    $hostAllowed = true;
}
if (!$hostAllowed) {
...
if (!function_exists('curl_init')) {
    ...
    header('Content-Type: text/plain');
    echo 'php-curl extension is required for the TRS-80 CORS proxy';
    exit;
}
...
curl_setopt($ch, CURLOPT_USERAGENT, 'IlluminatOS-TRS80-Proxy/1.0');
...
curl_setopt($ch, CURLOPT_WRITEFUNCTION, function ($_ch, $data) use ($state, $maxBytes) {
    ...
    if (!$state->headersSent) {
        ...
        if ($code >= 200 && $code < 300) {
            ...
            // safely cacheable for a short window; one TTL here keeps it
            // simple since metadata changes rarely.
