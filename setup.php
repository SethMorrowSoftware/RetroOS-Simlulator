...
if (!$isLocked && file_exists($envFile)) {
    ...
    // The canonical schema (backend/env.example.php, Database.php) uses the
    // 'database' key; wizard-generated files historically used 'db'. Check
    // both — reading only 'db' silently skipped this guard on every install
    // configured from env.example.php, letting an anonymous visitor re-run
    // the wizard. Fall back to other env secrets when the DB password is
    // empty so the guard still bites.
    $reconfirmSecret = '';
    if (is_array($existingEnv)) {
        foreach ([
            $existingEnv['database']['password'] ?? null,
            $existingEnv['db']['password'] ?? null,
            $existingEnv['migrations']['secret'] ?? null,
            $existingEnv['app']['internal_secret'] ?? null,
        ] as $candidate) {
            if (is_string($candidate) && $candidate !== '') {
                $reconfirmSecret = $candidate;
                break;
            }
        }
    }
    if ($reconfirmSecret !== '') {
        ...
        if (!is_string($reconfirmKey) || !hash_equals($reconfirmSecret, $reconfirmKey)) {
            http_response_code(403);
            ...
            echo '<p>To re-run setup, provide the current database password (or, if it is empty, the migration secret) as <code>?reconfirm_key=...</code> or restore the lock file at <code>config/.setup-complete</code>.</p>';
            echo '</div></body></html>';
            exit;
        }
    }
...
function handleDatabaseConfig(): array
{
    ...
    // Write env.php. Keys MUST match backend/env.example.php — that is the
    // schema the backend actually reads (Database.php: 'database',
    // FileStorageService: uploads.storage_path/max_size/user_quota,
    // Middleware: rate_limit.*, WebhookDispatcher: webhooks.request_timeout).
    // The wizard previously wrote 'db'/'upload_dir'/'timeout_seconds' and no
    // rate_limit block, which silently disabled rate limiting and broke the
    // re-run guard on wizard-provisioned installs.
    $envContent = "<?php\n";
    $envContent .= "/**\n * IlluminatOS! Backend Environment Configuration\n * Generated by setup wizard on " . date('Y-m-d H:i:s') . "\n */\n";
    $envContent .= "return [\n";
    $envContent .= "    'app' => [\n";
    $envContent .= "        'name'             => 'IlluminatOS!',\n";
    ...
    $envContent .= "    ],\n\n";
    $envContent .= "    'rate_limit' => [\n";
    $envContent .= "        'enabled'         => true,\n";
    $envContent .= "        'storage_path'    => __DIR__ . '/../data/rate_limits',\n";
    $envContent .= "        'fallback_window' => 60,\n";
    $envContent .= "        'fallback_limit'  => 60,\n";
    $envContent .= "    ],\n\n";
    $envContent .= "    'uploads' => [\n";
    $envContent .= "        'storage_path' => __DIR__ . '/../data/uploads',\n";
    $envContent .= "        'max_size'     => 10485760,\n";
    $envContent .= "        'user_quota'   => 104857600,\n";
    $envContent .= "        'allowed_types' => [\n";
    $envContent .= "            'image/png', 'image/jpeg', 'image/gif', 'image/webp',\n";
    $envContent .= "            'text/plain', 'text/csv',\n";
    $envContent .= "            'application/json', 'application/pdf',\n";
    ...
    $envContent .= "    'webhooks' => [\n";
    $envContent .= "        'enabled'         => true,\n";
    $envContent .= "        'request_timeout' => 5,\n";
    $envContent .= "        'max_retries'     => 3,\n";
    $envContent .= "        'retry_delay'     => 5,\n";
    $envContent .= "    ],\n\n";
    $envContent .= "    'cors' => [\n";
    $envContent .= "        'enabled'           => false,\n";
    $envContent .= "        'origins'           => [],\n";
    $envContent .= "        'allow_credentials' => true,\n";
    $envContent .= "    ],\n";
    $envContent .= "];\n";
...
if (file_exists($envFile)) {
    ...
    $existingDb = $existingEnv['database'] ?? $existingEnv['db'] ?? null;
    if (is_array($existingDb)) {
        $dbDefaults['db_host'] = $existingDb['host'] ?? '127.0.0.1';
        $dbDefaults['db_port'] = (string)($existingDb['port'] ?? 3306);
        $dbDefaults['db_name'] = $existingDb['database'] ?? '';
        $dbDefaults['db_user'] = $existingDb['username'] ?? '';
        $dbDefaults['db_pass'] = $existingDb['password'] ?? '';
    }
