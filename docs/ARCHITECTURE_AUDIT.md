...
| `StateManager` cache vs `StorageManager` cache vs module-level caches (e.g. `AchievementSystem._achievementsCache`) | `StateManager.js`, `StorageManager.js`, `AchievementSystem.js:62` | ✅ resolved — `setStateAndPersist` (PR #2) closes the state-vs-storage half; `AchievementSystem._achievementsCache` (the only module-level cache identified by the full-codebase sweep) was dropped — config achievements are read fresh on every access, runtime registrations live on the instance and are cleared in `cleanup()`. |
...
| Multi-instance apps must use `setInstanceState()` | `Terminal.js:31-57` (verified) violates this — class-level `commandHistory`, `currentPath`, `aliases`, `envVars` shared across windows — ✅ fixed in PR #2 (Wave 4). All 11 fields are now backed by property accessors that proxy to per-window state; `singleton: true` removed. A full audit of all 44 apps then identified `Paint.js` as the only other app holding per-window state on `this` (`ctx`, `tool`, `color`, `brushSize`, `lastX`, `lastY`, `painting`, `resizeObserver`, `_mpSession`, `_mpUnsubscribers`) — same prototype-accessor pattern applied. Every other app already passes the checklist. |
